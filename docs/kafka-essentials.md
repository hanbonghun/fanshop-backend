...
- 파티션은 리더 1 + 팔로워 N으로 복제된다. **ISR**(In-Sync Replicas) = 리더를 충분히 따라잡은 복제본 집합. **리더 자신도 ISR에 포함된다.**
- `acks=0` 안 기다림 / `acks=1` 리더만 / `acks=all` **ISR 전체**.
- 주의 — `acks=all`의 "all"은 **replication factor 전체가 아니라 그 시점의 ISR 전체**다. ISR이 줄면 기다리는 대상도 줄어든다.
- ISR에서 빠지는 기준은 `replica.lag.time.max.ms`(기본 30초). **다운뿐 아니라 느려도 빠진다** — GC가 길거나 디스크가 밀리면 ISR이 조용히 줄어 있을 수 있다.

### `min.insync.replicas` — 성공 조건이 아니라 거부 하한선

가장 많이 오해하는 지점. 이 값은 **"몇 개가 복제되면 성공인가"가 아니라 "몇 개 미만이면 아예 쓰기를 거부할 것인가"**를 정한다.

| 설정 | 어디 설정 | 무엇을 정하나 |
|---|---|---|
| `acks=all` | **프로듀서** | 몇 개의 ack를 기다릴지 → 현재 ISR 전체 |
| `min.insync.replicas` | **브로커/토픽** | ISR이 이 수 미만이면 쓰기를 **거부** |

**RF=3, `min.insync.replicas=2`일 때 실제 동작**

| 상황 | ISR 크기 | `acks=all`이 기다리는 대상 | 쓰기 |
|---|---|---|---|
| 정상 | 3 (리더 + 팔로워 2) | **3개 전부** | 성공 |
| 팔로워 1개 다운/지연 | 2 (리더 + 팔로워 1) | **2개** | 성공 |
| 팔로워 2개 다 빠짐 | 1 (리더만) | — | **거부** (`NotEnoughReplicasException`) |

즉 **평상시엔 3개를 다 기다린다.** "2개만 되면 OK"가 아니다. 하나가 빠지면 그때 비로소 2개만 기다리고(빠진 replica는 애초에 기다림의 대상이 아니다), 2 미만으로 떨어지면 쓰기를 막는다.

**왜 이 하한선이 필요한가** — `min.insync.replicas=1`이면 팔로워가 전부 빠져 리더 혼자 남아도 리더가 혼자 ack하고 성공을 돌려준다. `acks=all`이 사실상 `acks=1`로 퇴화한다. 그 직후 리더 디스크가 죽으면 **프로듀서는 성공 응답을 받았는데 데이터는 없다.** `=2`면 "최소 2벌이 존재함이 보장된 상태"에서만 성공을 돌려주므로 리더가 죽어도 데이터가 남는다.

```
RF=3, min.insync=1  →  1대 죽어도 쓰기 OK,  유실 가능      (내구성 X)
RF=3, min.insync=2  →  1대 죽어도 쓰기 OK,  유실 없음      ← 균형점
RF=3, min.insync=3  →  1대만 죽어도 쓰기 중단              (가용성 X)
```

`RF - min.insync = 감당 가능한 장애 노드 수`로 외운다.

**부수 조건 둘**

- **`acks=all`이 아니면 `min.insync.replicas`는 무시된다.** 브로커 설정과 프로듀서 설정이 짝을 이뤄야 효과가 난다. 한쪽만 걸면 아무 보장이 없다.
- `unclean.leader.election.enable=false`(기본값)여야 한다. true면 ISR 밖의 뒤처진 replica가 리더로 승격되어 위 보장이 무너진다.

### 이 프로젝트의 구성

- **브로커 1대, replication factor 1** → `acks=all`이어도 실질적으로 `acks=1`이다. 브로커 디스크가 죽으면 데이터는 없다.
- **여기에 `min.insync.replicas=2`를 걸면 모든 쓰기가 즉시 거부된다.** ISR이 항상 1이기 때문이다. RF ≥ 2일 때만 의미 있는 설정이다.
...
**Q. `min.insync.replicas=2`가 무슨 뜻인가?**
"2개만 복제되면 성공"이 아니라 **"ISR이 2 미만이면 쓰기를 거부한다"는 하한선**이다. ISR이 3이면 `acks=all`은 3개를 다 기다린다. 하나가 빠지면 2개를 기다리고, 1까지 떨어지면 `NotEnoughReplicasException`으로 쓰기를 막는다. 이게 없으면 리더 혼자 ack하고 성공을 돌려준 직후 리더가 죽어 유실될 수 있다.
